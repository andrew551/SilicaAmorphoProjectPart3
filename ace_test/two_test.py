...
def compute_u0(r):
    ...
    try:
        ...
        f = atoms.get_forces()
        f = np.linalg.norm(f[0, :]) * np.sign(np.dot(f[0, :], [1,0,0]))
    except Exception:
        u = math.nan
        f = math.nan
...
def test_all_potentials_dimer(specie = 'SiO'):
    ...
    for potential in potentials:
        ...
        atoms.calc = lammps
        MPI.COMM_WORLD.Barrier()
        result = [compute_u0(r) for r in rs]
        results[potential] = [x[0] for x in result]
        results_forces[potential] = [x[1] for x in result]
        close_lammps(lammps)
    ...
    plt.ylabel("u / eV")
    plt.xlabel("r / Å")
    plt.title(f"{specie} Dimer potentials comparison")
    plt.savefig(f"{specie}_Dimer_comparison_ACE_new_REF_LINEAR_ZBL.png", dpi=400)
    plt.ylim((1e-2, 1e8))
    plt.xlim((0, 1.2))
    ax.set_yscale('log')
    plt.savefig(f"{specie}_Dimer_comparison_ACE_new_REF_LINEAR_LOG_CROPPED_ZBL.png", dpi=400)

    fig, ax = plt.subplots()
    for potential, f in results_forces.items():
        linestyle = ':' if potential == 'CHIK' else '-'
        if potential == 'ACE_KAMIL_5' or potential=='ACE_REP1' or potential == 'ACE_CHEAP_CHUCK_4':
            linestyle = '--'
        if potential == 'ACE_REP1':
        ...
        if potential=='ACE_REP2':
            potential = 'ACE_REP0.5'
        plt.plot(rs, f, label = potential, linestyle = linestyle)
    plt.ylim((-20000, 50000))
    plt.legend()
    plt.ylabel('f')
    plt.xlabel('r / Å')
    plt.savefig(f"{specie}_Dimer_comparison_ACE_forces_LINEAR_ZBL.png", dpi=400)
    plt.ylim((-10, 10))
    plt.savefig(f"{specie}_Dimer_comparison_ACE_forces_LINEAR_ZBL_close.png", dpi=400)
...
if __name__ == '__main__':
    ...
    test_all_potentials_dimer(specie='SiO')
    test_all_potentials_dimer(specie='O2')
    test_all_potentials_dimer(specie='Si2')
